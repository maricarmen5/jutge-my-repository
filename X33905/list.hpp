...
public:
  ...
  void pop_back() {
    if (_size == 0) {
      cout << "Error: pop_back on empty list" << endl;
      exit(1);
    }
  ...
  void pop_front() {
    if (_size == 0) {
      cout << "Error: pop_front on empty list" << endl;
      exit(1);
    }
    removeItem(iteminf.next);
  }
  ...
  public:
    ...
    void operator++()
    {
      if (pitem == &(plist->itemsup)) {
		cout << "Error: ++iterator at the end of list" << endl;
		exit(1);
      }
      pitem = pitem->next;
    }
	...
	cout << "Error: --iterator at the beginning of list" << endl;
	exit(1);
      }
      pitem = pitem->prev;
    }
